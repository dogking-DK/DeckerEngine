...
| M2 场景文档 | 实体、变换、层级、资产引用、保存与重载 | M1 | 进行中：M2.1 完成 |
...
| M2.2 | 组件与变换层级 | M2.1 | 名称/Transform/Hierarchy；父子传播、循环拒绝与删除策略 | 待开始 |
| M2.3 | 工程格式与资产引用 | M2.2 | 格式版本与 AssetId 引用校验，缺失资源诊断 | 待开始 |
...
| 子阶段 | 范围 | 前置 | 验收与证据 | 状态 |
| --- | --- | --- | --- | --- |
| M3.1 | 命令注册与能力发现 | M2 | 参数/结果 schema、错误和未知命令有可执行验证 | 待开始 |
| M3.2 | 场景服务与编辑操作 | M3.1 | 命令完成实体和层级编辑，过期 revision 被拒绝 | 待开始 |
| M3.3 | 事务与撤销重做 | M3.2 | 一组内存编辑原子提交或回滚，撤销/重做保持状态 | 待开始 |
| M3.4 | CPU Runtime 与 CLI 批处理 | M3.3 | 无窗口进程可创建、保存、重启并查询场景 | 待开始 |
