...
| 子阶段 | 范围 | 前置 | 验收与证据 | 状态 |
| --- | --- | --- | --- | --- |
| M4.1 | 资产身份与导入契约 | M3 | 源文件/.meta/产物边界与版本明确，重命名保留 AssetId | 待开始 |
| M4.2 | 首个网格导入与 assetc | M4.1 | 支持的 glTF 子集可离线导入，非法数据有定位错误 | 待开始 |
| M4.3 | 缓存和依赖失效 | M4.2 | 输入/参数变化触发重建，删除缓存可恢复且 ID 不变 | 待开始 |
| M4.4 | 异步加载与任务生命周期 | M4.3 | 加载状态、等待/取消/退出有终态，CPU Ready 可用于运行时 | 待开始 |
